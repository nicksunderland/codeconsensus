...
ukbb_icd10 <- ukbb_icd10[, .(code = diag_icd10, desc = NA, code_type = "ICD10", count = .N), by = .(diag_icd10)][, diag_icd10 := NULL]
...
ukbb_opcs4 <- ukbb_opcs4[, .(code = oper4, desc = NA, code_type = "OPCS4", count = .N), by = .(oper4)][, oper4 := NULL]
...
# combine and save
nhs_counts <- rbind(nhs_icd10, nhs_opcs4, gp_snomed)
