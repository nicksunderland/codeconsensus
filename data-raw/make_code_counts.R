...
ukbb_icd10 <- ukbb_icd10[, .SD[1], by = .(eid, diag_icd10)][, .(icd10 = diag_icd10, count = .N), by = .(diag_icd10)][, diag_icd10 := NULL]
...
ukbb_opcs4 <- ukbb_opcs4[, .SD[1], by = .(eid, oper4)][, .(opcs4 = oper4, count = .N), by = .(oper4)][, oper4 := NULL]
...
nhs_icd10 <- rbind(
  data.table::as.data.table(readxl::read_xlsx("/Users/xx20081/Library/CloudStorage/OneDrive-UniversityofBristol/phenotyping/NHS_digital_code_counts/hosp-epis-stat-admi-diag-2022-23-tab_V2.xlsx", sheet = "All Diagnoses 4 Character", range = "A12:H11337"))[, .(code = `...1`, desc = `...2`, count = `...8`)],
  data.table::as.data.table(readxl::read_xlsx("/Users/xx20081/Library/CloudStorage/OneDrive-UniversityofBristol/phenotyping/NHS_digital_code_counts/hosp-epis-stat-admi-diag-2022-23-tab_V2.xlsx", sheet = "All Diagnoses 3 Character", range = "A12:I2051"))[, .(code = `...2`, desc = `...3`, count = `...9`)]
)
...
# https://digital.nhs.uk/data-and-information/publications/statistical/hospital-admitted-patient-care-activity/2022-23
nhs_opcs4 <- rbind(
  data.table::as.data.table(readxl::read_xlsx("/Users/xx20081/Library/CloudStorage/OneDrive-UniversityofBristol/phenotyping/NHS_digital_code_counts/hosp-epis-stat-admi-proc-2022-23-tab-V2.xlsx", sheet = "All Procedure 4 Character", range = "A11:H9034"))[, .(code = `...1`, desc = `...2`, count = `...8`)],
  data.table::as.data.table(readxl::read_xlsx("/Users/xx20081/Library/CloudStorage/OneDrive-UniversityofBristol/phenotyping/NHS_digital_code_counts/hosp-epis-stat-admi-proc-2022-23-tab-V2.xlsx", sheet = "All Procedure 3 Character", range = "A11:H1562"))[, .(code = `...1`, desc = `...2`, count = `...8`)]
)
...
gp_snomed <- gp_snomed[, .(code = SNOMED_Concept_ID, desc = Description, count = as.numeric(Usage))]
